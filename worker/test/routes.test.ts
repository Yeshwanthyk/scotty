...
const sandboxConfig = vi.hoisted(() => ({
  ...
  materializeEnvironment: vi.fn(),
  resolveCredentialForOrigin: vi.fn(),
...
describe("real Hono boundary", () => {
  beforeEach(() => {
    ...
    sandboxConfig.resolveCredentialForOrigin.mockImplementation(async (input: unknown) => {
      const origin =
        typeof input === "object" &&
        input !== null &&
        "origin" in input &&
        typeof input.origin === "string"
          ...
          : "";
      if (
        ["https://github.com", "https://api.github.com", "https://codeload.github.com"].includes(
          origin,
        )
      )
        return {
      ...
      if (origin === "https://api.openai.com")
        return {
          ok: true as const,
          value: { name: "OPENAI_API_KEY", scheme: "bearer", value: "real-openai-key" },
        };
      if (origin === "https://opencode.ai" || origin === "https://api.opencode.ai")
        return {
          ok: true as const,
          value: { name: "OPENCODE_API_KEY", scheme: "bearer", value: "real-opencode-key" },
        };
