...
const retainedSummary = (index: number): EvidenceJobSummary => ({
  sequence: index,
...
const retainedArtifact = (jobId: string): EvidenceArtifact => ({
  sessionId: SESSION_ID,
  jobId,
  frameId: "old-frame",
  objectKey: `evidence/${SESSION_ID}/${jobId}/old-frame.png`,
  mediaType: "image/png",
  sha256: "a".repeat(64),
  ...
  it.effect("owns the deterministic manifest across every upload seam", () =>
    Effect.gen(function* () {
      ...
      assert.deepInclude(authority.readEvidence()?.artifacts[0], {
        objectKey: `evidence/${SESSION_ID}/job-1/frame-1.png`,
      ...
      assert.strictEqual(artifact.objectKey, `evidence/${SESSION_ID}/job-1/frame-1.png`);
