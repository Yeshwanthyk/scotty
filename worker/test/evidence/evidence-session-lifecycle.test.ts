  ...
  for (let jobIndex = EVIDENCE_MAX_RETAINED_JOBS - 1; jobIndex >= 0; jobIndex -= 1) {
    ...
    for (let frameIndex = 0; frameIndex < EVIDENCE_MAX_STEPS; frameIndex += 1) {
      ...
      artifacts.push({
        ...
        jobId,
        frameId,
        objectKey: `evidence/${SESSION_ID}/${jobId}/${frameId}.png`,
        mediaType: "image/png",
        sha256: jobIndex.toString(16).padStart(2, "0").repeat(32),
    ...
    artifacts.push({
      ...
      jobId,
      frameId: "recording",
      objectKey: `evidence/${SESSION_ID}/${jobId}/recording.webm`,
...
describe("evidence session lifecycle", () => {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        assert.lengthOf(harness.artifactKeys(), 1);
        const state = harness.read<EvidenceState>(sessionHarnessKeys.evidence);
  ...
  it.effect("preserves the pre-evidence lifecycle exactly when the gate is disabled", () =>
    Effect.gen(function* () {
      yield* TestClock.setTime(NOW);
      const clock = yield* Clock.Clock;
      const malformedEvidence = { activeJob: "malformed" };
  ...
  it.effect("evicts one full job of artifacts at the retention boundary before acceptance", () =>
    Effect.gen(function* () {
      ...
      const accepted = harness.read<EvidenceState>(sessionHarnessKeys.evidence);
  ...
  it.effect("blocks boundary acceptance until an ambiguous eviction is confirmed", () =>
    Effect.gen(function* () {
      ...
      const pending = harness.read<EvidenceState>(sessionHarnessKeys.evidence);
  ...
  for (const scheduleFailure of [
    ...
    it.effect(`keeps ambiguous retention deletes actionable after ${scheduleFailure}`, () =>
      Effect.gen(function* () {
        ...
        const reconciled = harness.read<EvidenceState>(sessionHarnessKeys.evidence);
  ...
  it.effect("serializes concurrent preview admission at four persisted permits", () =>
    Effect.gen(function* () {
      ...
      assert.strictEqual(
        harness.read<EvidenceState>(sessionHarnessKeys.evidence)?.activeJob?.previewAccounting
          .permits.length,
        4,
      );
    }),
  ...
  it.effect("revokes the digest before ambiguous unexpose and retains finalizing authority", () =>
    Effect.gen(function* () {
      ...
      const pending = harness.read<EvidenceState>(sessionHarnessKeys.evidence)?.activeJob;
