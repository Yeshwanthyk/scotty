...
class MaterializerFilesystemFake {
  ...
  private async applyExec(command: string): Promise<ExecResult> {
    const archiveResult = await this.applyArchiveCommand(command);
    if (archiveResult !== undefined) return archiveResult;
    const tarResult = this.applyTarCommand(command);
    if (tarResult !== undefined) return tarResult;
    return this.applyFilesystemCommand(command);
  }

  private async applyArchiveCommand(command: string): Promise<ExecResult | undefined> {
  ...
  private applyTarCommand(command: string): ExecResult | undefined {
    const failed = (): ExecResult => ({ ...successResult(command), success: false, exitCode: 1 });
  ...
  private applyFilesystemCommand(command: string): ExecResult {
...
const emptyBundle = () => createDeterministicTarGz([fileMember("manifest.json", '{"items":[]}\n')]);
...
describe("SandboxBundleMaterializer", () => {
  ...
  it.effect("materializes the item list and preserves each item shape", () =>
    Effect.gen(function* () {
      const filesystem = new MaterializerFilesystemFake();
      const bundle = makeMemoryBundleCapabilities();
