...
describe("credential protocol contracts", () => {
  ...
  it("keeps registry envelopes and RPC contracts bounded and non-secret", () => {
    ...
    assert.ok(Option.isNone(decodeResolvedCredentialOption({ value: "" })));
    assert.ok(Option.isNone(decodeResolvedCredentialOption({ value: "x".repeat(256 * 1024 + 1) })));
    assert.ok(
      Option.isSome(
