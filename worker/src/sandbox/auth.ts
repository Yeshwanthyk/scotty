...
export const sandboxAgentsInstructions = scottyBaseAgentInstructions;
...
export const containerAuthLayer: Layer.Layer<ContainerAuth, never, SandboxRuntime> = Layer.effect(
  ...
  Effect.map(SandboxRuntime, (runtime) => {
    ...
    const startPiSession = Effect.fnUntraced(function* (
      ...
      const agentsPath = `${sessionRoot(id)}/.pi-agent/AGENTS.md`;
      yield* runtime.writeFile(agentsPath, configuration.agentInstructions);
      yield* runtime.execChecked(`chmod 600 ${shellQuote(agentsPath)}`);
    ...
    return ContainerAuth.of({
      ...
      ensurePiSession: Effect.fnUntraced(function* (id, credentials, selection, configuration) {
        const existing = yield* runtime.getProcess(PI_SESSION_PROCESS_ID);
        if (existing?.status === "starting" || existing?.status === "running") {
          yield* waitForPiSessionReady(id);
          return;
        }
        yield* startPiSession(id, credentials, selection, configuration);
        yield* waitForPiSessionReady(id);
