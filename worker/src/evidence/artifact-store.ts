...
type ArtifactMediaType = EvidenceArtifact["mediaType"];
...
interface ArtifactStoreShape {
  ...
  ) => Effect.Effect<EvidenceArtifact, EvidenceArtifactError>;
  ...
  readonly deleteFrame: (artifact: EvidenceArtifact) => Effect.Effect<void, EvidenceArtifactError>;
...
const expectedMetadata = (
  input: Pick<EvidenceArtifact, "sessionId" | "jobId" | "frameId">,
  sha256: string,
...
const canonicalArtifact = (artifact: EvidenceArtifact): boolean =>
...
const makeArtifactStore = (capabilities: ArtifactStoreCapabilities): ArtifactStoreShape => {
  ...
  const openArtifact = Effect.fnUntraced(function* (artifact: EvidenceArtifact) {
  ...
  const deleteArtifact = Effect.fnUntraced(function* (artifact: EvidenceArtifact) {
