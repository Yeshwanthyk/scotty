...
export interface EvidenceWorkflowControlShape {
  ...
  ) => Effect.Effect<ExposedEvidencePreview, EvidenceWorkflowControlError>;
  ...
  readonly completeVideo: (
    active: EvidenceActiveJob,
    input: CompleteEvidenceVideoPublication,
  ) => Effect.Effect<void, EvidenceWorkflowControlError>;
  ...
  ) => Effect.Effect<EvidenceJobSummary, EvidenceWorkflowControlError>;
}
...
const activeMatchesJob = (active: EvidenceActiveJob, job: BrowserEvidenceJob): boolean =>
