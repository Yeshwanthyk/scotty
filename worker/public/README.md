...
On phone-sized viewports, the client uses a native textarea composer for agent prompts. The browser handles selection, paste, dictation, autocorrection, and IME composition; submitting pastes the complete draft through Ghostty's bracketed-paste support, sends Enter, and dismisses the software keyboard. The compact floating composer follows the visual viewport's position and size, so Safari's focus scroll keeps it docked above the software keyboard instead of jumping to the document top. Delayed viewport reads cover WebKit's late keyboard metrics without resizing the PTY rows. The leading composer control morphs between keyboard and dismiss actions without changing columns, and the keyboard button exposes exactly four one-tap controls: Esc, Tab, Ctrl-C, and Enter. New output preserves manual scrollback until the reader taps `Latest`.
