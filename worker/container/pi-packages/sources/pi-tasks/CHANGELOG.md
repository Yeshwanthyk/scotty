...
### Changed
- Stale-task reminders now use submitted request cycles, fire only for actionable tasks, and restart their cadence after reminder injection or task-tool use.
- Persisted completed tasks rejoin automatic cleanup countdowns on resume/startup restoration and when switching from `never` to an automatic mode.
- Empty default session stores are removed without recreating their JSON file; high-water-mark sidecars remain intact, while named/explicit `PI_TASKS` stores are never treated as session defaults.
...
- Task lifecycle completion/deletion side effects now run only for accepted status transitions, so rejected and repeated updates cannot restart cleanup countdowns.
