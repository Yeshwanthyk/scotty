...
- The Session Sandbox Durable Object owns the Session record, operation lease, immutable credential-generation grant, sentinel vault, backup handles, and hard-cap metadata until Vaporize. The installation Credential Durable Object owns encrypted named credential generations and OAuth refresh; Account Secrets Store owns only the Installation wrapping key. SandboxConfig owns the sanitized Pi seed and repository registry. KV is a non-secret list projection. R2 contains immutable backups. Container files and Effect runtime memory are never authoritative.
...
- Real Installation credentials must never enter container env/files/process args/logs/Git config/KV/R2/API responses or Alchemy props, outputs, and state. Credential-object storage contains ciphertext only; typed brokers and bridges hold plaintext for one authorized operation. Containers receive Session-bound sentinels only. OAuth refresh commits before the sanitized response returns.
