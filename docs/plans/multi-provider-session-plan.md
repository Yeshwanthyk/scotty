...
6. Prove a CLI-only upgrade with the deployed container image digest unchanged, including failed-selection retention and stale-completion rollback prevention. Automatic newest-compatible selection does not introduce an activation/rollback endpoint. Only after live proof and a separate approved rollout may image compilation/copy/version inventory and obsolete CLI-only Docker-context inputs be removed from `worker/container/Dockerfile`, `worker/container/toolsets/standard.json`, `cli/src/deployment-packaging.{ts,mjs}`, and affected checks. Preserve inputs shared with native Codex or other image-owned tools. Pi/Codex runtimes, native wrappers and toolchains remain image-owned.
