...
Status: rebuild acceptance specification. The Sandbox Durable Object remains authoritative. The
rebuild owns one strict browser contract at `/api/sessions/:id`; it does not accept the old Worker
view or preserve a compatibility route. React, TanStack Start, TanStack Router, and StyleX own
...
| `Stable(Gone)`                   | Hidden from lists                                | Terminal tombstone on a stale deep link; no runtime content fetch | None                                   |

The browser contract exposes only `authority.kind = stable | transitioning`. Stable lifecycle is
`warm | sleeping | failed | gone`. A transition has an `action`, `phase`, and
`mode = executing | reconciling`; the actions are `create`, `checkpoint`, `sleep`, `resume`,
`work`, `evidence`, `hatch`, `down`, and `vaporize`. Labels such as Creating and Vaporized are display copy,
never aliases in types, fixtures, routes, or semantic data attributes. Agent `working`, `waiting`,
`completed`, and `tool-stalled` are expiring activity observations, not session lifecycle states.
...
4. A Pi checkpoint/stream 409 means conversation runtime content is temporarily unavailable while
...
| `many-sessions`       | 4 repositories, 60 sessions, long/duplicate titles, all non-terminal lifecycle labels, keyboard and scroll retention                          |
...
| ------------------------ | ------------------------------------------------------------------- | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| Home/rail                | empty, many, stale projection, all visible stable/transition labels | create, click every row, keyboard open, back/forward, rapid A/B/A switch           | Screenshots + navigation video + route/assertion JSON              |
