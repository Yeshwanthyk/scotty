...
const missingEnabledPluginRules = [...enabledRules]
  .filter((rule) => !exportedRules.has(rule))
  .sort();
...
for (const rule of [...enabledRules].sort()) {
  ...
  for (const match of source.matchAll(
    /\b(?:modeling|decoding|deriving|inferring|wrapping|testing|maintaining|routing)-[a-z0-9-]+\b/gu,
  )) {
...
if (
  ...
  missingPluginRules.length > 0 ||
  missingEnabledPluginRules.length > 0 ||
  ...
  if (missingEnabledPluginRules.length > 0)
    problems.push(
      `Enabled rules missing from plugin exports: ${missingEnabledPluginRules.join(", ")}`,
    );
