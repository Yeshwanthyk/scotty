//! Mends half-streamed inline Markdown for display.
//!
//! Adapted from Comet's `crates/ui/src/markdown/mend.rs` at commit
//! `b033110d087ae0f1d1ba607b77d97624165c1986` (itself inspired by
//! streamdown's `remend`).
//!
...
//! moment content follows an opener. Only the display parse should see mended
//! text. The canonical Markdown source must remain untouched, so a marker that
//! genuinely never closes settles honestly — one flip when the row completes —
//! instead of jittering throughout.
//!
//! Repairs, in the spirit of remend's handler set (its katex/html/comparison
//! handlers don't apply to Scotty's renderer):
...
//! port suitable for an incremental parser.
...
pub const PENDING_LINK_URL: &str = "scotty:pending-link";
...
mod tests {
    ...
    fn links_mend_to_pending_sentinel() {
        mends("[docs](https://x.dev/lo", "[docs](scotty:pending-link)");
        mends("[docs](", "[docs](scotty:pending-link)");
        mends("see [do", "see [do](scotty:pending-link)");
        mends("![alt](https://x/i.p", "![alt](scotty:pending-link)");
        stays("see [");
    ...
    fn link_urls_allow_nested_parens() {
        ...
        mends("[a](https://x.dev/(y", "[a](scotty:pending-link)");
    }

    #[test]
    fn emphasis_inside_link_text_closes_inside() {
        mends("[**a", "[**a**](scotty:pending-link)");
        mends("**a [b", "**a [b](scotty:pending-link)**");
    }
