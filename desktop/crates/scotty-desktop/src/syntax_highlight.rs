//! Lightweight, dependency-free, paint-only syntax tokenizer.
//!
//! Keyword, string, comment, and number classes are scanned line-by-line with a
//! small carry state for constructs spanning lines. Callers apply the resulting
//! byte ranges as color-only highlights to their existing text.
...
mod tests {
    ...
    #[test]
    fn utf8_text_keeps_token_ranges_on_character_boundaries() {
        let line = "let café = \"東京\"; // résumé";
        let tokens = tokenize_line(Lang::Rust, line, LineCarry::None).0;

        assert!(tokens.iter().all(|token| {
            line.is_char_boundary(token.range.start) && line.is_char_boundary(token.range.end)
        }));
        assert!(tokens.iter().any(|token| {
            &line[token.range.clone()] == "\"東京\"" && token.class == TokenClass::StringLit
        }));
    }
