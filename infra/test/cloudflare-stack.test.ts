...
import { makeInstallationTopology } from "../installation.ts";
import { PREBUILT_MAIN_WORKER_ENTRY } from "../../cli/src/prebuilt-worker-bundles.ts";
import { readOwnedPreviewTopologyDeletion } from "../preview-ownership.ts";

const source = readFileSync(new URL("../cloudflare-stack.ts", import.meta.url), "utf8");
...
const previewInstallation = makeInstallationTopology("home", {
...
describe("Cloudflare stack guard", () => {
  ...
  it("requires explicit preview authority when evidence is enabled", () => {
    const enabledWithoutPreview = makeInstallationTopology("home", undefined, true);
    assert.throws(
      () =>
        assertCloudflareStackConfig({
          ...approvedConfig(),
